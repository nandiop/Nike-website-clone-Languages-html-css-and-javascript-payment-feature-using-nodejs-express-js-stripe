...
function renderCartItems() {
  ...
  minusButton.forEach((button, i) => {
    button.addEventListener("click", function () {
      ...
      let quantities = JSON.parse(localStorage.getItem("quantities")) || {};
      quantities[i] = quantity; // Save the quantity for this specific item
      localStorage.setItem("quantities", JSON.stringify(quantities)); // Update the input value
    });
  });
...
checkoutButton.addEventListener("click", async (event) => {
  ...
  cartItems.forEach((item, index) => {
    const quantity =
      JSON.parse(localStorage.getItem("quantities"))?.[index] || 1;
...
// Utility function to parse and retrieve quantities from localStorage
const getQuantitiesFromLocalStorage = () => {
  try {
    const quantity = JSON.parse(localStorage.getItem("quantities"));
   
    //return Array.isArray(quantity) ? quantity : [];
    return quantity;
  } catch (error) {
    console.error("Error parsing quantities from localStorage:", error);
    return [];
  }
}; // Function to calculate total price and total items
const calculateCartTotals = (cartItems) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      let checkoutTotalPrice = 0;
      let totalItems = 0;
      const quantities = getQuantitiesFromLocalStorage();
      ...
      resolve({
        displayTotalPrice:
          displayTotalPrice > 0 ? `$ ${displayTotalPrice.toFixed(2)}` : "$0",
        checkoutTotalPrice,
        totalItems,
      });
    } catch (error) {
      reject(new Error("Error calculating cart totals: " + error.message));
    }
  });
}; // Example usage
const updateCartDisplay = async () => {
  try {
    ...
    const { displayTotalPrice, totalItems } = await calculateCartTotals(
      cartItemsArray
    ); // Assuming these elements exist in the DOM
    
    
    
  
    if (totalPriceElement && totalItemElement) {
      totalPriceElement.textContent = displayTotalPrice;
      totalItemElement.textContent = totalItems;
    } else {
...
updateCartDisplay();
//totalPriceElement.textContent =  displayTotalPrice>0 ? `$ ${displayTotalPrice}` : `$0`;
